function crop_edges_dates(mask)
%%%acts on int file
...
for l=1:length(rlooks)
  ...
  for k=1%:ndates    
    if(exist(dates(k).unwrlk{1},'file'))
      fidi=fopen(dates(k).unwrlk{1},'r');
      fido=fopen([dates(k).unwrlk{1} 'tmp'],'w');
      for j=1:newny(l)
	...
	if(and(j>=id3,j<=id4))
      ...
      fclose(fido);
      fclose(fidi);
movefile([dates(k).unwrlk{1} 'tmp'],dates(k).unwrlk{1});
    end
  end
end
