...
[nx,ny,lambda] = load_rscs(dates(id).slc,'WIDTH','FILE_LENGTH');
...
for l=1:length(rlooks)
    ...
    if(flagr)
        ...
        rate = fread(fidr,[newnx(l),newny(l)],'real*4');
        rate = rate'/lambda*(4*pi)/1000/365; %convert rate back to radians/day
    else
        rate = zeros(newny(l),newnx(l));  
    end
