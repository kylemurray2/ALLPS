...
masterdir = '/data/rlohman/testalos/'; %For now, set this by hand, make sure it exists, and run from this dir.
track     = 224; %track for ERS/ENVI, path for ALOS
frame     = [820]; %frame for ERS/ENVI, row for ALOS.  Can be vector of multiple adjacent frames
sat       = 'ALOS';  %ERS,ENVI,ALOS or TSX
% 
  % track         = 213;
  % frame         = [2907];
...
datamode = 3; %1 = download, 2=already downloaded, in masterdir
plotflag = 1;
...
%should fix to not have to make ints
make_ints
make_ramps
flat_ints
FilterFlat
...
crop_edges([71*4 32*4 20*12 10*12]); %full res location of crop (so that it works for multiple rlooks, left right, top, bottom.
